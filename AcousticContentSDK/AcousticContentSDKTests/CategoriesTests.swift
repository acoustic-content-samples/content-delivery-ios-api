...
class CategoriesTypesTests: XCTestCase {
    
    var testRequestSession = AcousticTestContext.getSessionMock(.category)
    ...
    func testNoError() {
        ...
        deliverySearch.categories().get() { (result) in
    ...
    func testClassificationIsCategory() {
        let expectation = XCTestExpectation()
        deliverySearch.categories().get() { (result) in
            XCTAssert(result.documents.first?.classification == "category")
            expectation.fulfill()
        }
        wait(for: [expectation], timeout: AcousticTestContext.timeout)
    }
